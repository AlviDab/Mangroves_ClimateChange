...
selected <- map(prioritisations_list, function(prior_name) {
  ...
  selected_cn <- solution %>%
    st_drop_geometry() %>%
    filter(solution_1 == 1) %>%
    mutate(type = paste("Climate-naïve",
                        scale_indicator,
  ...
  selected_cs <- solution_cc %>%
    st_drop_geometry() %>%
    filter(solution_1 == 1) %>%
    mutate(type = paste("Climate-smart",
                        scale_indicator,
                        sensitivity_indicator),
...
theme_layer <- list(theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1),
...
ggsave(plot = ggplot_area, "Figures/resp_v1/01_comparison_area.jpg",
       dpi = 300, width = 18, height = 15, units = "cm")

# Resilience
ggsave(plot = ggplot_resilience, "Figures/resp_v1/01_comparison_resilience.jpg",
       dpi = 300, width = 18, height = 15, units = "cm")
