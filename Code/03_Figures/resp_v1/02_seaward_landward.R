...
map(split_group_names, function(split_group) {
  ...
  kappa_by_country <- map(unique(solution_cc_lw$country), function(sel_country) {
    ...
    unlist() %>%
    tibble(country = unique(solution_cc_lw$country),
           kappa = .)
  ...
  dir.create(
    paste0(
      "Figures/resp_v1/Country/09_map_kappa_ratio_lw_sw/",
      split_group,
      "/RDS"
    ),
    recursive = TRUE
...
# I now patchwork them together, at the top I will put the Area difference (%) legend and then the agreement legend, with two rows
library(patchwork)
...
# Save it as svg with the other plots
ggsave(
  "Figures/resp_v1/Country/09_map_kappa_ratio_lw_sw/map_ratio_lw_sw.svg",
  plot = plot_map,
  dpi = 300,
  width = 18,
  height = 22,
