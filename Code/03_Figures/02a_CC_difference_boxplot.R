...
# Date: 02/10/2024
# Description: Boxplot of climate resilience in selected areas by country
...
# Mapping over CC directions
map(c(
  "landward", "seaward",
  "mean"
), function(CC_direction) {
  # Select column name based on CC direction
  ...
  # Calculate total area of mangroves in PUs
  total_area <- PUs %>%
  ...
  selected_summary <- selected %>%
    group_by(type) %>%
    summarise(
      weighted_mean_exposure = weighted.mean(
        !!sym(col_name),
        area_km2
      ),
    ...
    ) # How to calculate std error in weighted mean?

  # Calculate weighted mean exposure by country
  selected_country <- selected %>%
    left_join(PUs %>%
      dplyr::select(c("country", "cellID"))) %>%
    group_by(country, type) %>%
    summarise(weighted_mean_exposure = weighted.mean(
      !!sym(col_name),
      area_km2
    )) %>%
  ...
  # Calculate differences between approaches by country
  selected_country_difference <- selected_country %>%
    dplyr::select(country, type, weighted_mean_exposure) %>%
    pivot_wider(names_from = type, values_from = weighted_mean_exposure) %>%
    mutate(
      diff_global_scale = abs(`Climate-naïve Global-scale` - `Climate-smart Global-scale`),
      diff_country_scale = abs(`Climate-naïve Country-scale` - `Climate-smart Country-scale`),
      diff_climate_smart = abs(`Climate-smart Country-scale` - `Climate-smart Global-scale`)
    )
  ...
  map(c( # "all_comparisons",
    ...
    plot_comparison <- ggplot() +
      ...
      geom_errorbar(
        data = comparison_summary, aes(
          x = factor(type, level = level_order), y = weighted_mean_exposure,
      ...
      geom_point(
        data = comparison, aes(
          y = weighted_mean_exposure, x = factor(type, level = level_order),
          colour = factor(type, level = level_order),
    ...
    dir.create(paste0("Figures/Country/02a_resilience/RDS"), recursive = TRUE)

    # Save the plot
    ggsave(
      plot = plot_comparison, paste0(
        "Figures/Country/02a_resilience/boxplot_resilience_",
    ...
    saveRDS(plot_comparison, paste0(
      "Figures/Country/02a_resilience/RDS/boxplot_resilience_",
