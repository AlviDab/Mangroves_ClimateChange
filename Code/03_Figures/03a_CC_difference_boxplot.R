...
selected_country_difference <- selected_country %>%
  ...
  mutate(diff_global_scale = abs(`Climate-naïve Global-scale` - `Climate-smart Global-scale`),
...
map(c(#"all_comparisons", "Global-scale",
  "Country-scale"
  #, "Climate-smart"
), function(group_name) {
  ...
  plot_comparison <- ggplot() +
    ...
    #            size = 4) +
    geom_errorbar(data = comparison_summary, aes(x = factor(type, level = level_order), y = weighted_mean_exposure,
                                                 ymin = weighted_mean_exposure,
                                                 ymax = weighted_mean_exposure,
                                                 colour = factor(type, level = level_order)),
                  width = 0.6,
                  linewidth = 1.2) +
    geom_point(data = comparison, aes(y = weighted_mean_exposure, x = factor(type, level = level_order),
                                      colour = factor(type, level = level_order),
  ...
  dir.create(paste0("Figures/Country/03a_resilience/RDS"), recursive = TRUE)

  ggsave(plot = plot_comparison, paste0("Figures/Country/03a_resilience/boxplot_resilience_",
                                        CC_direction, "_", prct, "_", group_name , ".pdf"),
         dpi = 300, width = 18, height = 8, units = "cm")

  saveRDS(plot_comparison, paste0("Figures/Country/03a_resilience/RDS/boxplot_resilience_",
