...
map(c("targets_30", "targets_area"), function(targets) {
  map(c("country_and_biotyp", "biotyp"), function(split_group) {
    ...
    dir.create(paste0("Figures/resp_v1/Country/02_area/",
    ...
    writeLines(paste0("Percentage increase in total area climate-smart prioritisation compared to climate naive = ", as.character(diff/area_cn$area_km2[2]*100), "%"),
               paste0("Figures/resp_v1/Country/02_area/",
    ...
    ggsave(plot = barplot_area, paste0("Figures/resp_v1/Country/02_area/",
                                       split_group, "_", targets, "/barplot_area_",
                                       CC_direction, "_", prct, ".pdf"),
           dpi = 300, width = 12, height = 8, units = "cm")

    saveRDS(barplot_area, paste0("Figures/resp_v1/Country/02_area/",
