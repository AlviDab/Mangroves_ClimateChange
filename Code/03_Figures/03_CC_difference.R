...
map(c("targets_30", "targets_area"), function(targets) {
  map(c("country_and_biotyp", "biotyp"), function(split_group) {
    ...
    solution <- readRDS(paste0("Results/resp_v1/RDS/prioritisation/Country/01_prioritisation/",
                               split_group, "_", targets,"/solution_prioritisation.rds"))

    solution_cc <- readRDS(paste0("Results/resp_v1/RDS/prioritisation/Country/02_prioritisation_CC/",
    ...
    dir.create(paste0("Figures/resp_v1/Country/03_CC_exposure/", split_group, "_", targets, "/RDS"), recursive = TRUE)

    ggsave(plot = kd_plot, paste0("Figures/resp_v1/Country/03_CC_exposure/",
                                  split_group, "_", targets, "/kdplot_exposure_",
                                  CC_direction, "_", prct, ".pdf"),
           dpi = 300, width = 12, height = 12, units = "cm")

    saveRDS(kd_plot, paste0("Figures/resp_v1/Country/03_CC_exposure/", split_group, "_", targets,
    ...
    write.xlsx(selected %>%
                 st_drop_geometry(), paste0("Figures/resp_v1/Country/03_CC_exposure/",
                                            split_group, "_", targets, "/kdplot_exposure_",
...
The value reported in the name of the file is the percentage used as a trade-off to select climate-priority areas for each conservation feature (more on the method 'climate-priority areas' in Buenafe et al. 2023 - https://doi.org/10.1002/eap.2852).",
           "Figures/resp_v1/Country/03_CC_exposure/info.txt")
