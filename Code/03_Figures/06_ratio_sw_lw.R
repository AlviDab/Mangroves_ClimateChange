...
# Date: 24/06/2024
# Description: This script creates a map showing the ratio of area selected by landward vs
#             seaward prioritisation solutions for each country (landward - seaward)
#             and the agreement (Kappa) between the two solutions.
...
# Calculate the ratio of area selected by landward vs seaward prioritisation solutions for each country
solution_cc_separate <- solution_cc_sw %>%
  ...
  pivot_wider(names_from = "solution_1", values_from = c(
    "tot_area",
    "w_mean_cc_exp_sw",
    "w_mean_cc_exp_lw"
  )) %>%
...
# Join landward and seaward solutions and the kappa value
...
# Save outputs
dir.create(paste0("Figures/Country/06_map_kappa_ratio_lw_sw/", split_group, "_", targets, "/RDS"), recursive = TRUE)

# Save the plot
ggsave(
  paste0(
    "Figures/Country/06_map_kappa_ratio_lw_sw/",
    split_group, "_", targets, "/map_ratio_lw_sw_", prct, ".pdf"
  ),
...
write.xlsx(
  ...
  file = paste0(
    "Figures/Country/06_map_kappa_ratio_lw_sw/",
    split_group, "_", targets, "/map_kappa_ratio_lw_sw_", prct, ".xlsx"
  )
)
...
saveRDS(plot_map, paste0(
  "Figures/Country/06_map_kappa_ratio_lw_sw/",
  split_group, "_", targets, "/RDS/map_ratio_lw_sw_", prct, ".rds"
))
# })
