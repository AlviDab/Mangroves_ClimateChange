...
area_cr <- map(c(#"landward", "seaward",
  "mean"), function(CC_direction) {
    ...
    area_cr_global <- area_cr %>%
      filter(type == "biotyp") %>%
      filter(prct_increase_area != 0)

    area_cr_country <- area_cr %>%
      filter(type == "country_and_biotyp") %>%
      filter(prct_increase_area != 0)

    fit_global <- lm(prct_increase_mean_resilience*100 ~ log(prct_increase_area*100),
                     data = area_cr_global)
    fit_country <- lm(prct_increase_mean_resilience*100 ~ log(prct_increase_area*100),
                      data = area_cr_country)
    ...
    scatterplot <- ggplot(data = area_cr, aes(x = prct_increase_area*100,
      ...
      geom_function(fun = function(x) fit_global$coefficients[1] +
                    ...
                    colour = '#2a9d8f') +
      geom_function(fun = function(x) fit_country$coefficients[1] +
                      fit_country$coefficients[2]*log(x),
                    colour = '#F4A261') +
